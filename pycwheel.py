...
# This module is modified by Harrison Feng which doesn't version info for
# compiled wheel file.
...
def convert_wheel(whl_file):
    ...
    The new whl file will be generated under ./compiled directory.
    """
    ...
    for py_file in glob.glob("{0}/**/*.py".format(whl_name)):
        print("Deleting py file: {0}".format(py_file))
        os.remove(py_file)
    ...
    for root, dirs, files in os.walk(whl_name):
        for f in files:
            if f.endswith(".py"):
                ...
                print("Removing file: {0}".format(py_name))
                os.remove(py_name)
    ...
    with open(record_path, "r") as record:
        for file_dest, hash, length in csv.reader(record):
            if file_dest.endswith(".py"):
                ...
                hash_value = "{0}={1}".format(HASH_TYPE,
...
def rezip_whl(whl_name):
    ...
    shutil.make_archive(whl_name, 'zip', whl_name)
...
def main(args=None):
    parser = argparse.ArgumentParser(
        description='Compile all py files in a wheel')
